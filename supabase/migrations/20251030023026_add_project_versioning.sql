/*
  # Add Project Versioning System

  1. New Tables
    - `project_versions`
      - `id` (uuid, primary key)
      - `project_id` (uuid, foreign key to projects)
      - `version_number` (text): Version identifier (v1.0, v2.0, etc.)
      ...
      - `total_amount` (decimal): Calculated total for this version
      - `created_at` (timestamptz)
      - `created_by` (text): User who created this version
      - `based_on_version_id` (uuid): Reference to parent version if duplicated

    - `version_project_areas`
      - Copy of project_areas structure linked to version_id
      - Allows multiple versions of areas per project

    - `version_area_cabinets`
      - Copy of area_cabinets structure linked to version area
      - Stores cabinet configurations per version

    - `version_area_items`
      - Copy of area_items structure linked to version area
      - Stores items per version

  2. Migration Strategy
    - Create version tables
    - Create initial v1.0 version for all existing projects
    - Migrate existing data to v1.0 versions
  ...
  4. Security
    - RLS enabled on all version tables
    - Public access for now (can be restricted with auth later)
...
SELECT
  ac.id, ac.area_id, ac.product_id, ac.quantity,
  ac.box_material_id, ac.box_material_name, ac.box_material_cost,
  ac.box_edgeband_id, ac.box_edgeband_name, ac.box_edgeband_cost,
  ac.box_interior_finish_id, ac.box_interior_finish_name, ac.box_interior_finish_cost,
  ac.doors_material_id, ac.doors_material_name, ac.doors_material_cost,
  ac.doors_edgeband_id, ac.doors_edgeband_name, ac.doors_edgeband_cost,
  ac.doors_interior_finish_id, ac.doors_interior_finish_name, ac.doors_interior_finish_cost,
  ac.hardware, ac.hardware_cost, ac.labor_cost, ac.subtotal,
  ac.product_sku, ac.product_description, ac.box_sf, ac.doors_fronts_sf, ac.total_edgeband, ac.has_drawers, ac.is_rta,

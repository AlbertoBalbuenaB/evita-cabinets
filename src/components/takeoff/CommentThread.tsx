  ...
  return (
    <Modal isOpen onClose={close} title={root.resolved ? 'Comment (resolved)' : 'Comment'} size="md">
      <div className="space-y-3">
        ...
        {replies.length > 0 && (
          <div className="border-l-2 border-border-soft pl-3 space-y-2">
        ...
        {!root.resolved && (
          <div>
            ...
            />
            <p className="text-[10px] text-fg-400 mt-0.5">⌘↵ / Ctrl+↵ to post</p>
          </div>
        )}
...
function CommentBlock({ comment, isRoot }: { comment: TakeoffComment; isRoot?: boolean }) {
  return (
    <div className={isRoot ? 'bg-surf-app rounded-lg p-3' : ''}>

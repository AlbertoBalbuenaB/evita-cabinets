    ...
    >
      <div className="flex items-start gap-2 mb-2">
        ...
        <div className="min-w-0 flex-1">
          <h3 className="text-sm font-semibold text-fg-900">{title}</h3>
          {summary && (
            <p className="text-xs text-fg-600 mt-1 line-clamp-2">{summary}</p>
          )}
        </div>
      </div>
      <div className="flex items-center gap-2 text-xs text-fg-500 mt-2 flex-wrap">
      ...
      {article.tags.length > 0 && (
        <div className="flex flex-wrap gap-1 mt-2">
          ...
          {article.tags.length > 5 && (
            <span className="text-[10px] text-fg-400">+{article.tags.length - 5}</span>
          )}
        </div>
      )}
    </Link>

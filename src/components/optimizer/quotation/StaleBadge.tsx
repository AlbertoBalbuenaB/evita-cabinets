...
export function StaleBadge({ onRerun, size = 'sm' }: Props) {
  ...
  return (
    <button
      ...
      onClick={onRerun}
      disabled={!onRerun}
      className={`inline-flex items-center gap-1 ${padding} ${text} rounded-full bg-status-amber-bg text-amber-800 font-medium ${onRerun ? 'hover:bg-amber-200 transition-colors cursor-pointer' : 'cursor-default'}`}

...
export function PerAreaBoardsBreakdown({ rows }: Props) {
    ...
    return (
      <div className="rounded-lg border border-slate-200 bg-white p-4">
        <div className="flex items-center gap-1.5 mb-2">
          ...
          <h3 className="text-xs font-semibold text-slate-800 uppercase tracking-wide">Cost by Area</h3>
        </div>
        <p className="text-xs text-slate-400 italic">No areas to display. Build and run the optimizer first.</p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white">
      <div
        className="flex items-center gap-1.5 px-3 py-2 border-b border-slate-200"
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.areaId} className="border-b border-slate-50 last:border-b-0">
                <td className="px-3 py-1.5 text-slate-800 font-medium">{r.areaName}</td>
                <td className="px-3 py-1.5 text-right tabular-nums text-slate-600">{r.m2.toFixed(2)}</td>
                <td className="px-3 py-1.5 text-right tabular-nums text-slate-600">{r.boards.toFixed(1)}</td>
                <td className="px-3 py-1.5 text-right tabular-nums text-slate-800">{formatCurrency(r.cost)}</td>
              </tr>
            ))}
          </tbody>
          <tfoot>
            <tr className="bg-slate-50 font-semibold">
              ...
              <td className="px-3 py-1.5 text-right tabular-nums text-slate-800">{totals.m2.toFixed(2)}</td>
              <td className="px-3 py-1.5 text-right tabular-nums text-slate-800">{totals.boards.toFixed(1)}</td>
              <td className="px-3 py-1.5 text-right tabular-nums text-slate-800">{formatCurrency(totals.cost)}</td>
            </tr>
          </tfoot>
        </table>
      </div>

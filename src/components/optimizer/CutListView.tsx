...
export function CutListView({ result }: Props) {
  ...
  if (!result) return <div className="flex-1 flex items-center justify-center text-slate-400 text-sm">Ejecuta una optimización para ver la lista de corte</div>;
  return (
        ...
        return (
          <div key={boardIdx} className="mb-6">
            <div className="bg-slate-50 rounded-t-lg border border-slate-200 px-4 py-2 font-medium text-sm text-slate-700">
              Tablero {boardIdx + 1} • {board.material} {fmtDim(board.grosor, unit)} • {fmtDim(board.ancho, unit)}×{fmtDim(board.alto, unit)} • Uso: {board.usage.toFixed(1)}%
            </div>
            <table className="w-full border border-t-0 border-slate-200 text-xs">
              <thead className="bg-slate-50 border-b border-slate-200">
                <tr>{['#','Nombre',`Ancho(${unitLabel(unit)})`,`Alto(${unitLabel(unit)})`,`X(${unitLabel(unit)})`,`Y(${unitLabel(unit)})`,'Rot','Veta','Canto'].map(h => <th key={h} className="px-3 py-2 text-left font-medium text-slate-500">{h}</th>)}</tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {board.placed.map((p, i) => {
                  const edgeBands = [p.piece.cubrecanto.sup?'S':'', p.piece.cubrecanto.inf?'I':'', p.piece.cubrecanto.izq?'L':'', p.piece.cubrecanto.der?'R':''].filter(Boolean).join('/');
                  return (
                    <tr key={i} className="hover:bg-slate-50">
                      ...
                      <td className="px-3 py-1.5 text-center text-slate-500">{p.rotated ? 'Sí' : '—'}</td>
                      <td className="px-3 py-1.5 text-center"><span className={`px-1.5 py-0.5 rounded text-xs font-medium ${p.piece.vetaHorizontal ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-500'}`}>{p.piece.vetaHorizontal ? 'Fija' : 'Libre'}</span></td>
            ...
            {cuts.length > 0 && (
              <div className="mt-2 mb-1 ml-2 space-y-0.5">
                <div className="text-xs font-semibold text-slate-500 mb-1">Secuencia ({cuts.length} cortes):</div>
                {cuts.map(cut => (

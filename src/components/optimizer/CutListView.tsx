                ...
                {board.placed.map((p, i) => {
                  ...
                  return (
                    <tr key={i} className="hover:bg-surf-app">
                      ...
                      <td className="px-3 py-1.5 text-center"><span className={`px-1.5 py-0.5 rounded text-xs font-medium ${p.piece.veta !== 'none' ? 'bg-amber-100 text-amber-700' : 'bg-surf-muted text-fg-500'}`}>{p.piece.veta === 'none' ? '—' : p.piece.veta === 'horizontal' ? '↔' : '↕'}</span></td>

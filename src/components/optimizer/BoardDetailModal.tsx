    ...
    <Modal isOpen={isOpen} onClose={onClose} title={`Board ${boardIndex + 1} — ${board.material} ${board.grosor}mm`} size="xl">
      <div className="flex gap-4 h-full min-h-0">
        ...
        <div className="w-52 shrink-0 space-y-4">
          <div>
            <h4 className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-2">Board</h4>
          ...
          <div>
            <h4 className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-2">Pieces ({board.placed.length})</h4>
            <div className="space-y-1 max-h-48 overflow-y-auto">
              {board.placed.map((p, i) => (
                <div key={i} className="flex items-center gap-2 text-xs">
                  ...
                  <span className="text-slate-600 flex-1 truncate">{p.piece.nombre || `Piece ${i + 1}`}</span>

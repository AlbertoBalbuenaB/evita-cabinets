...
export function StatsBar({ result }: Props) {
  const effColor = !result ? 'text-fg-300' : result.efficiency > 75 ? 'text-green-600' : result.efficiency > 50 ? 'text-amber-600' : 'text-red-600';
    ...
    <div className="bg-surf-card border-b border-border-soft overflow-x-auto flex-shrink-0">
      ...
      <div className="flex">
        ...
        <div className="flex-1 px-4 py-2.5 border-r border-border-soft min-w-max">
          ...
          <div className={`text-xl font-bold ${unplaced.length > 0 ? 'text-red-600' : 'text-fg-900'}`}>

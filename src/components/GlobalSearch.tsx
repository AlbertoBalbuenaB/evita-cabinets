...
export function GlobalSearch({ open, onClose }: GlobalSearchProps) {
  ...
  return (
    ...
    >
      {/* Modal panel */}
      <div
        className="w-full sm:max-w-2xl bg-surf-card rounded-2xl shadow-2xl overflow-hidden flex flex-col"
      ...
      >
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-border-soft">
          <Search className="h-5 w-5 text-fg-400 flex-shrink-0" />
          <input
            ref={inputRef}
            type="text"
            value={query}
            ...
            className="flex-1 bg-transparent outline-none text-fg-800 placeholder:text-fg-400 text-base"
            autoComplete="off"
            spellCheck={false}
          />
          <div className="flex items-center gap-2 flex-shrink-0">
            {isLoading && <Loader2 className="h-4 w-4 text-fg-400 animate-spin" />}
            {query.length > 0 && !isLoading && (
              <button
                onClick={() => setQuery('')}
                className="p-1 rounded-md text-fg-400 hover:text-fg-600 hover:bg-surf-muted transition-colors"
                tabIndex={-1}
            ...
            <kbd className="hidden sm:inline-flex items-center px-1.5 py-0.5 text-xs font-mono text-fg-400 bg-surf-muted rounded border border-border-soft">
              Esc
            </kbd>
          </div>
        </div>
        ...
        <div ref={listRef} className="overflow-y-auto flex-1">
          {/* Prompt state */}
          {showPrompt && (
            <div className="px-4 py-10 text-center text-fg-400 text-sm">
              <Search className="h-8 w-8 mx-auto mb-2 opacity-30" />
              <p>Type at least 2 characters to search</p>
              <p className="text-xs mt-1 text-fg-300">
          ...
          {showEmptyState && (
            <div className="px-4 py-10 text-center text-fg-400 text-sm">
              <Search className="h-8 w-8 mx-auto mb-2 opacity-30" />
              <p>No results for &ldquo;<span className="text-fg-600 font-medium">{query}</span>&rdquo;</p>
            </div>
          )}
              ...
              return (
                <div key={key}>
                  ...
                  <div className="flex items-center gap-2 px-4 pt-3 pb-1">
                    ...
                    <span className="text-xs font-semibold uppercase tracking-wider text-fg-400">
                      {config.label}
                    </span>
                    <span className="text-xs text-fg-300 ml-auto">{items.length}</span>
                  </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {result.subtitle && (
                            <p className="text-xs text-fg-400 truncate mt-0.5">
          ...
          {/* Footer hint */}
          {hasResults && (
            <div className="flex items-center gap-4 px-4 py-2 border-t border-border-soft mt-1">
              <span className="text-xs text-fg-300 flex items-center gap-1">
                <kbd className="inline-flex items-center px-1 py-0.5 text-xs font-mono bg-surf-muted rounded border border-border-soft text-fg-400">↑↓</kbd>
                navigate
              </span>
              <span className="text-xs text-fg-300 flex items-center gap-1">
                <kbd className="inline-flex items-center px-1 py-0.5 text-xs font-mono bg-surf-muted rounded border border-border-soft text-fg-400">↵</kbd>
              ...
              <span className="text-xs text-fg-300 flex items-center gap-1">
                <kbd className="inline-flex items-center px-1 py-0.5 text-xs font-mono bg-surf-muted rounded border border-border-soft text-fg-400">Esc</kbd>
                close
              </span>
            </div>
          )}

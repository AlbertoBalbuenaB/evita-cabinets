...
export function KbMarkdownViewer({ source, className }: KbMarkdownViewerProps) {
  ...
  const components: Components = {
      ...
      if (isInternal) {
        return (
          <button
            ...
            className="text-indigo-600 hover:text-indigo-800 underline underline-offset-2 font-medium bg-transparent border-0 p-0 inline cursor-pointer"
          >
            {children}
          </button>
        );
    ...
    h4: ({ children }) => <h4 className="text-base font-semibold text-slate-800 mt-3 mb-1.5">{children}</h4>,
    p: ({ children }) => <p className="text-slate-700 leading-relaxed my-2">{children}</p>,
    ul: ({ children }) => <ul className="list-disc list-outside ml-6 space-y-1 my-2 text-slate-700">{children}</ul>,
    ol: ({ children }) => <ol className="list-decimal list-outside ml-6 space-y-1 my-2 text-slate-700">{children}</ol>,
    li: ({ children }) => <li className="leading-relaxed">{children}</li>,
    blockquote: ({ children }) => (
      <blockquote className="glass-indigo border-l-4 border-indigo-400 px-4 py-2 my-3 text-slate-700 italic rounded-r-lg">
        {children}
      </blockquote>
    ),
    code: ({ className, children }) => {
      ...
      return (
        <code className="px-1.5 py-0.5 rounded bg-indigo-50 text-indigo-700 text-sm font-mono">{children}</code>
      );
    },
    ...
    thead: ({ children }) => <thead className="bg-slate-50/60">{children}</thead>,
    tbody: ({ children }) => <tbody>{children}</tbody>,
    tr: ({ children }) => <tr className="border-b border-slate-200/60">{children}</tr>,
    th: ({ children }) => <th className="text-left px-3 py-2 font-semibold text-slate-800">{children}</th>,

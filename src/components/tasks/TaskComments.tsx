...
const MentionSuggestionList = forwardRef<SuggestionListRef, SuggestionListProps>(
  ({ items, command }, ref) => {
    ...
    if (!items.length) {
      return (
        <div className="bg-surf-card border border-border-soft rounded-xl shadow-lg p-3 w-60 text-center">
          <p className="text-xs text-fg-400">No results</p>
        </div>
      );
    }
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 text-sm transition-colors ${
                      isSelected ? 'bg-blue-50 text-blue-900' : 'text-fg-700 hover:bg-surf-app'
                    }`}
                  >
                    @{item.label}
...
function CommentEditor({ mentionItems, placeholder, onSubmit, onCancel, compact }: EditorProps) {
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: 'prose prose-sm max-w-none focus:outline-none min-h-[60px] px-3 py-2 text-sm text-fg-700',
      },
    },
  });
  ...
  return (
    <div className="border border-border-soft rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-blue-500 focus-within:border-transparent">
      ...
      <div className="flex items-center justify-end gap-2 px-2 py-1.5 bg-surf-app border-t border-border-soft">
        {onCancel && (
          <button onClick={onCancel} className="text-xs text-fg-400 hover:text-fg-600">Cancel</button>
        )}
        <button
          onClick={submit}
          className="flex items-center gap-1 text-xs font-medium text-blue-600 hover:text-blue-700 disabled:text-fg-300"
        >
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2">
        <span className="text-[10px] text-fg-400">Commenting as:</span>
        {showAuthorInput ? (
          <div className="flex items-center gap-1">
            <input
              ...
              autoFocus
              className="text-xs border border-border-soft rounded px-2 py-0.5 w-32 focus:outline-none focus:ring-1 focus-visible:ring-focus"
            />
          </div>
        ) : (
          <button
          ...
          <div className="flex items-start gap-2 group">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-baseline gap-2 mb-1">
                <span className="text-xs font-semibold text-fg-700">
                ...
                <div key={reply.id} className="flex items-start gap-2 group">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-baseline gap-2 mb-0.5">
                      <span className="text-xs font-semibold text-fg-600">

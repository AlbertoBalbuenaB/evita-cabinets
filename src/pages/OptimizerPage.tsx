...
export function OptimizerPage() {
  ...
  // ── Excel import (Planilla Evita / Planilla Masisa) ───────
    ...
    <div className="h-[calc(100vh-3.5rem)] flex flex-col overflow-hidden bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-200 px-4 py-2.5 flex items-center gap-2 flex-shrink-0 flex-wrap">
        ...
        <input
          ...
          className="px-2 py-1 text-sm border border-slate-200 rounded focus:ring-2 focus:ring-blue-500 w-28"
        />
        <input
        ...
        <div className="flex items-center rounded border border-slate-200 overflow-hidden text-xs font-medium">
          <button
            onClick={() => store.setUnit('mm')}
            className={`px-2.5 py-1 transition-colors ${store.unit === 'mm' ? 'bg-blue-600 text-white' : 'bg-white text-slate-500 hover:bg-slate-50'}`}
          >mm</button>
          <button
            onClick={() => store.setUnit('in')}
            className={`px-2.5 py-1 transition-colors ${store.unit === 'in' ? 'bg-blue-600 text-white' : 'bg-white text-slate-500 hover:bg-slate-50'}`}
          >in</button>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden min-h-0">
        ...
        {/* Left panel: sidebar (stocks, options) + piece list */}
        <div className="w-72 shrink-0 flex flex-col border-r border-slate-200 bg-white overflow-y-auto overflow-x-hidden">
          <OptimizerSidebar />
          <PieceList pieces={store.pieces} onRemove={store.removePiece} />
        </div>

        {/* Center panel: interactive CAD viewer */}
        <CADViewer
          board={selectedBoard}
          unit={store.unit}
        />

        {/* Right panel: global stats + sheet stats + cuts */}
        <RightStatsPanel
          result={store.result}
